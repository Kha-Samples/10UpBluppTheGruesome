...
class Inventory {
	...
	public static function init() {
		items = new Array();
		selected = -1;
		y = Empty.height - itemHeight - 2 * spacing;
	}
	...
	public static function paint(g: Graphics) {
		...
		var itemY = y + spacing;
		g.color = Color.Black;
		g.fillRect(0, y, Empty.width, itemHeight + 2 * spacing);
		g.color = Color.White;
		...
		if (Player.current() != null) {
			...
			if (thing != null) {
				...
				g.drawString("Press E to use " + thing, Empty.width - g.font.width(g.fontSize, text) - 10, y + (itemHeight + 2 * spacing) / 2 - g.font.height(g.fontSize) / 2);
			}
		}
	}
}
